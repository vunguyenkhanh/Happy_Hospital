...
export class Constant {
  ...
  public static get SAFE_DISTANCE(): number {
    return 46
  }
  ...
  public static minDistance(actor: Actor, otherActors: Set<Actor>): number {
    ...
    otherActors.forEach((element) => {
      let smaller = Math.sqrt(
        (element.x - actor.x) ** 2 + (element.y - actor.y) ** 2
      )
