...
export class Forcasting {
  ...
  public addDuration(id: number, duration: WaitingDuration): void {
    ...
    let m: Set<WaitingDuration> = this.waitingAutoAgv.get(
      id
    ) as Set<WaitingDuration> //.add(duration);
    m.add(duration)
  ...
  public totalAverageWaitingTime(): number {
    ...
    this.waitingAutoAgv.forEach((value: Set<WaitingDuration>, key: number) => {
  ...
  public log(text: Phaser.GameObjects.Text): void {
    ...
    text.setText(
      'Tu giay: ' +
        Math.floor(performance.now() / 1000 - Constant.DELTA_T) +
        ', #AutoAgv: ' +
        numAutoAgv +
        ' totalTime: ' +
        total +
        ...
        result +
        '#Stop: ' +
        this.waitingAutoAgv?.get(2)?.size
    )
  }
